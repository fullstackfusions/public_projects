...
| [`langfuse_observability_lab`](./projects/langfuse_observability_lab/) | AI / LLM / Observability | LangGraph, LangChain, MCP, Langfuse, FastAPI, Ollama | Hands-on Langfuse tracing for a supervisor -> subagent -> MCP-tools pipeline (ops, support, network-CLI agents): nested trace trees, token usage/cost, timeouts, sessions, trace scoring, an opaque-vs-traced retry comparison, and a WebSocket server for live-streamed progress with mid-flight steering and soft/hard cancel. |
