...
| [`langfuse_observability_lab`](./projects/langfuse_observability_lab/) | AI / LLM / Observability | LangGraph, LangChain, MCP, Langfuse, Ollama | Hands-on Langfuse tracing for a supervisor -> subagent -> MCP-tools pipeline (ops, support, network-CLI agents): nested trace trees, token usage/cost, timeouts, sessions, trace scoring, and an opaque-vs-traced retry comparison for silently-dropped gateway/MCP connections. |
